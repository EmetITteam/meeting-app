    // --- ЕДИНЫЙ, ОБЪЕДИНЕННЫЙ И ИСПРАВЛЕННЫЙ JAVASCRIPT-КОД ---

    // Глобальные переменные и константы
    const API_URL = "/api/handler";
    // APP_SECRET_TOKEN — set via Vercel env var APP_SECRET_TOKEN
    // Frontend reads it from a meta tag injected at build, or uses a public config endpoint.
    // For now: stored in sessionStorage after first /api/config fetch (see initAppToken below)
    let APP_TOKEN = '';
...
function getCategoryColor(categoryName) {
    if (!categoryName || categoryName.toLowerCase().includes('без категории')) return '#A0AEC0'; // Нейтральный серый

    const lowerCategory = categoryName.toLowerCase();

    // Новая сбалансированная палитра
    if (lowerCategory.includes('новый')) return '#4A90E2';       // Современный синий
    if (lowerCategory.includes('активный')) return '#48BB78';   // Насыщенный зеленый
    if (lowerCategory.includes('спящий')) return '#F2C94C';     // Теплый янтарный
    if (lowerCategory.includes('без закупок')) return '#F2994A'; // Мягкий оранжевый
    if (lowerCategory.includes('потерянный')) return '#EB5757';  // Спокойный красный
    
    return '#A0AEC0'; // Нейтральный серый по умолчанию
}
// --- КОНЕЦ БЛОКА НА ЗАМЕНУ ---
    // --- НАЧАЛО БЛОКА НА ЗАМЕНУ ---
...
async function callApi(action, payload) {
    try {
        ...
        if (!response.ok) {
            ...
            const errorMessage = `Ошибка API от сервера 1С: Статус ${response.status} для действия "${action}". Ответ: ${errorText}`;

            // --- Вручную отправляем ошибку сервера в Sentry ---
            if (window.Sentry) { // Проверяем, что Sentry инициализирован
                 Sentry.captureMessage(errorMessage, 'error'); // Отправляем как важное сообщение
            }

            throw new Error(errorMessage); // Продолжаем стандартную обработку ошибки
        }

        const result = await response.json();

        // ПРОВЕРКА ЛОГИЧЕСКОЙ ОШИБКИ ВНУТРИ 1С
        if (result.status === 'error') {
            const errorMessage = `Ошибка бизнес-логики 1С для действия "${action}": ${result.message}`;

            // --- Вручную отправляем ошибку бизнес-логики в Sentry ---
             if (window.Sentry) {
                 Sentry.captureMessage(errorMessage, 'warning'); // Отправляем как предупреждение
            }
        ...
        // Здесь ловятся сетевые ошибки (сервер недоступен) или ошибки разбора JSON

        // --- Отправляем исключение в Sentry ---
        if (window.Sentry) {
            Sentry.captureException(e); // captureException лучше подходит для перехвата исключений
        }
...
function formatElapsedTime(ms) {
    ...
    function showForm(show) {
        ...
        // --- ВСТАВЬТЕ ЭТОТ БЛОК В САМЫЙ КОНЕЦ СКРИПТА ---

// ГЛОБАЛЬНЫЙ ТАЙМЕР ДЛЯ ОБНОВЛЕНИЯ СЧЕТЧИКОВ

setInterval(() => {
    document.querySelectorAll('.timer').forEach(timerEl => {
        const startTime = parseInt(timerEl.dataset.startTime, 10);
        if (startTime) {
            timerEl.textContent = formatElapsedTime(Date.now() - startTime);
        }
    });
}, 1000);
    }
...
// Mock clients with contracts (primary contract marked with isPrimary)
...
// Exchange rate — updated daily from 1C (НБУ rate)
...
const MOCK_ORDERS = [
    ...
    },
    {
        ...
        contractName: 'Основний з 2020, грн.', contractCurrency: 'UAH',
        totalAmountUSD: 201, // 2×$67.5 + 1×$66
    ...
    },
    {
        ...
        totalAmountUSD: 321.7, // 3×$74.7 + 2×$48.8
...
// Mock catalog — prices in USD (shown as $X · Y грн at today's rate)
const MOCK_CATALOG = [
...
function renderOrders(filter) {
    ...
    if (orders.length === 0) {
        container.innerHTML = `<div style="text-align:center;padding:48px 0;color:var(--text3);">
            <i class="bi bi-bag" style="font-size:48px;display:block;margin-bottom:12px;"></i>
            <div style="font-size:15px;font-weight:700;">Замовлень не знайдено</div></div>`;
        return;
    }

    const stripeMap = { order_draft: 'gray', order_formed: 'blue', realization: 'orange' };
    const deliveryLabels = { courier: 'Кур\'єр', pickup: 'Самовивіз', nova_poshta: 'Нова Пошта', manager: 'Менеджер' };
    const payLabels = { cash: 'нал', cashless: 'безнал', deferred: 'відстрочка' };
    ...
    container.innerHTML = orders.map(o => {
        ...
        // Manager can only view — no status buttons
        const editBtn = (!o.posted) ? `<button class="card-btn js-order-edit" data-id="${o.id}"><i class="bi bi-pencil"></i> Редагувати</button>` : '';
        ...
        return `<div class="order-card">
            ...
            <div class="order-body">
                ...
                <div class="order-meta-row">
                    <span class="order-meta"><i class="bi bi-truck"></i> ${deliveryLabels[o.deliveryType] || o.deliveryType}</span>
                    ${o.deliveryAddress ? `<span class="order-meta"><i class="bi bi-geo-alt"></i> ${o.deliveryAddress}</span>` : ''}
                </div>
                <div class="order-amount">
                    $\u202f${o.totalAmountUSD % 1 === 0 ? o.totalAmountUSD : o.totalAmountUSD.toFixed(1)}
                    <span style="font-size:13px;font-weight:700;color:var(--text2);">· ${Math.round(o.totalAmountUSD * TODAY_RATE_USD).toLocaleString('uk-UA')}\u202fгрн</span>
                    <span>· ${payLabels[o.paymentType] || ''}</span>
                </div>
                <div class="order-footer">
                    <button class="card-btn card-btn-primary js-order-view" data-id="${o.id}"><i class="bi bi-eye"></i> Деталі</button>
                    ${editBtn}
                </div>
            </div>
        </div>`;
...
function showOrderDetail(order) {
    const deliveryLabels = { courier: 'Кур\'єр', pickup: 'Самовивіз', nova_poshta: 'Нова Пошта', manager: 'Менеджер' };
    const payLabels = { cash: 'Готівка', cashless: 'Безнал', deferred: 'Відстрочка' };
    ...
    const itemsHtml = order.items.map(it => {
        ...
        <div class="d-flex justify-content-between align-items-start py-2" style="border-bottom:1px solid var(--border);gap:10px;">
            <div style="flex:1;min-width:0;">
                <div style="font-size:13px;font-weight:700;color:var(--text);">${it.isGift ? '<span class="item-tag-gift"><i class="bi bi-gift"></i> Подарунок</span> ' : ''}${it.name}</div>
    ...
    document.getElementById('order-detail-content').innerHTML = `
        <div class="form-section-wrap mb-3">
            ...
            <div class="mb-2"><span style="font-size:12px;font-weight:700;color:var(--text3);">Клієнт:</span>
                <div style="font-size:14px;font-weight:700;">${order.clientName}</div>
            </div>
            <div><span style="font-size:12px;font-weight:700;color:var(--text3);">Договір:</span>
        ...
        <div class="form-section-wrap mb-3">
            <div class="mb-2"><span style="font-size:12px;font-weight:700;color:var(--text3);">Доставка:</span>
                <div style="font-size:13px;font-weight:700;">${deliveryLabels[order.deliveryType] || order.deliveryType}${order.deliveryAddress ? ' · ' + order.deliveryAddress : ''}</div>
            </div>
            <div><span style="font-size:12px;font-weight:700;color:var(--text3);">Оплата:</span>
                <div style="font-size:13px;font-weight:700;">${payLabels[order.paymentType] || order.paymentType}</div>
            </div>
            ${order.comment ? `<div class="mt-2"><span style="font-size:12px;font-weight:700;color:var(--text3);">Коментар:</span>
...
function openOrderEdit(order) {
    ...
    // Populate contracts for this client
...
function initAddOrderForm() {
        ...
        results.innerHTML = matches.map(p => `
            <div class="cart-item js-add-to-cart" data-id="${p.id}" style="cursor:pointer;${p.restricted ? 'opacity:.6;' : ''}">
                ...
                <div class="cart-item-right">
                    ...
                    ${!p.restricted ? `<button class="card-btn card-btn-primary" style="pointer-events:none;white-space:nowrap;">+ Додати</button>` : `<span style="font-size:11px;color:var(--red);font-weight:700;">Заборонено</span>`}
                </div>
            </div>`).join('');
    });
    ...
    // Cancel new order
    document.getElementById('cancel-new-client-btn'); // reuse existing cancel
}
...
function renderOrderCart() {
    ...
    // Check mock gift condition: if Vitaran qty >= 4 → show gift banner
...
function resetOrderForm() {
    orderCart = [];
    renderOrderCart();
    const fields = ['order-client-name','order-client-id','order-delivery-address','order-date-assembly','order-date-delivery','order-comment','order-event'];
...
function switchPageById(pageId) {
    ...
    const headerTitle = document.getElementById('header-title');
    if (headerTitle) {
        const titles = { orders: 'Замовлення', dashboard: 'Зустрічі', clients: 'Клієнти', analytics: 'Дашборд', 'add-order': 'Нове замовлення' };
...
function initFabSheet() {
    ...
    btnMeeting?.addEventListener('click', () => {
        closeSheet();
        // Trigger existing "add meeting" flow
        document.querySelector('.nav-button[data-page="dashboard"]')?.click();
        setTimeout(() => showAddForm?.(), 50);
    });
...
function renderDebtors(filter = 'all', search = '') {
    ...
    // Summary
    const allData = filter === 'all' && !search ? MOCK_DEBTORS : data;
    ...
    list.innerHTML = data.map(d => {
        ...
        const dueFmt    = d.dueDate ? new Date(d.dueDate).toLocaleDateString('uk-UA', {day:'2-digit',month:'short',year:'numeric'}) : '—';
        return `
            ...
            <div class="debtor-body">
                ...
                <div class="debtor-footer">
                    <div>
                        ...
                        ${isOverdue && d.overdue < d.total ? `<div style="font-size:11px;color:var(--text3);margin-top:1px;">прострочено: ${d.overdue.toLocaleString('uk-UA')}\u202fгрн</div>` : ''}
                    </div>
                    <div>
...
function initDebtorsPage() {
    ...
    backBtn?.addEventListener('click', () => {
        // Go back to previous active page — close offcanvas just navigated from
        window.history.back?.();
        switchPageById('dashboard');
    });
    ...
    document.getElementById('nav-debtors-link')?.addEventListener('click', e => {
        ...
        renderDebtors('all');
        // Reset filter chips
        document.querySelectorAll('#debtors-filter-chips .chip').forEach(c => c.classList.toggle('active', c.dataset.filter === 'all'));
        if (searchEl) searchEl.value = '';
    });
}

// ── INIT on DOMContentLoaded (appended to existing) ──
document.addEventListener('DOMContentLoaded', () => {
    renderOrders('all');
    initOrdersPage();
    initAddOrderForm();
